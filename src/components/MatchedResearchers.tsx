...
export default function MatchedResearchers({ projectId }: { projectId: string }) {
  ...
  useEffect(() => {
    const fetchResearchers = async () => {
      try {
        // const apiUrl = `${process.env.NEXT_PUBLIC_API_URL}/matching-results?project_id=${projectId}`; //本番環境用は・・・NEXT_PUBLIC_AZURE_API_URL}matching・・・に変更すること
        const apiUrl = `${process.env.NEXT_PUBLIC_AZURE_API_URL}matching-results?project_id=${projectId}`; //ローカル用は・・・NEXT_PUBLIC_API_URL}/matching・・・に変更すること
